...
function changeCity(id,s){
    ...
    for (let i=0; i<spz.length;i++){
        document.getElementById("stadt_spz_pics").innerHTML=document.getElementById("stadt_spz_pics").innerHTML+ "<img id=\"spz_pics_" + i + "\" src=\"alleindiestadt/spz/" + spz[i] + ".png\" class=\"spz_img\" onClick=\"stadtImg2(" + i + "," + id + ")\"></img><span id=\"stadt_spz_" + i + "\" class=\"textcaps\" style=\"cursor: pointer;\" onClick=\"stadtImg2(" + i + "," + id + ")\">" + spz[i] + "</span><br>"
    }
    ...
    if(s==1){
        window.scrollBy({top: document.getElementById("stadt_content").getBoundingClientRect().top, left: 0, behavior: 'smooth'});
    }
}
